...
> [!NOTE]
> If you are looking for the original template based on [this blog post](https://srid.ca/rust-nix)'s use of `crate2nix`, browse from [this tag](https://github.com/srid/nufetch/tree/crate2nix). The evolution of this template can be gleaned from [releases](https://github.com/srid/nufetch/releases).

## Usage

You can use [omnix](https://omnix.page/om/init.html)[^omnix] to initialize this template:
```
nix --accept-flake-config run github:juspay/omnix -- init github:gignsky/nufetch -o ~/my-nix-project
```
...
![CI Nix](https://github.com/gignsky/nufetch/actions/workflows/ci-nix.yml/badge.svg)](https://github.com/gignsky/nufetch/actions/workflows/ci-nix.yml)
[![Docs](https://github.com/gignsky/nufetch/actions/workflows/docs.yml/badge.svg)](https://github.com/gignsky/nufetch/actions/workflows/docs.yml)
[![Release](https://github.com/gignsky/nufetch/actions/workflows/release.yml/badge.svg)](https://github.com/gignsky/nufetch/actions/workflows/release.yml)
[![Update README with --help Output](https://github.com/gignsky/nufetch/actions/workflows/help-to-readme.yml/badge.svg)](https://github.com/gignsky/nufetch/actions/workflows/help-to-readme.yml)
...
`$ nix run github:gignsky/nufetch -- {optional-arguments (i.e. --help)}`

## Installation
### With Nix Shell (temporary installation in current shell)
`$ nix shell github:gignsky/nufetch`

### With NixOS (using flakes)
In your `flake.nix` file: 
1. add `inputs.nufetch.url = "github:gignsky/nufetch";` to your inputs
...
3. add either:
   1. `inputs.nufetch.packages.${system}.nufetch`
   2. `inputs.nufetch.packages.${system}.default`

### Download the binary
1. Navigate to https://github.com/gignsky/nufetch/releases
...
1. git clone this repository:
   1. `$ git clone https://github.com/gignsky/nufetch.git`
...
4. build binary with justfile (uses nix build command) or build with cargo
   1. `$ just build`
      1. nix build command
      2. produces a binary in `./result/bin/nufetch`
   2. `$ cargo build`
      1. cargo build command (debug)
      2. produces a debug binary in `./target/debug/nufetch`
   3. `$ cargo build --release`
      1. cargo build command (release)
      2. produces a debug binary in `./target/release/nufetch`
